const Carousel = () => {
  ...
  return (
    <div className="w-full min-h-screen bg-white flex flex-col justify-center overflow-hidden space-y-3 sm:space-y-4">
      {/* Top Row */}
      <div className="overflow-hidden">
        <div className="flex w-max animate-left">
          {[...topRow, ...topRow, ...topRow].map((item, index) => (
            <div
              key={index}
              className="w-[180px] sm:w-[220px] md:w-[280px] h-[90px] sm:h-[110px] md:h-[140px] bg-white rounded-md mx-1 flex items-center justify-center text-black border-2 text-xl sm:text-2xl md:text-4xl font-bold flex-shrink-0"
            >
              {item}
            </div>
          ))}
